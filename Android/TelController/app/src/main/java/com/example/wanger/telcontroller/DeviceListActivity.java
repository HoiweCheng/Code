...
public class DeviceListActivity extends AppCompatActivity {
    ...
    private enum BT_SEARCH_STATE{IDLE, SEARCHING};
    private BluetoothAdapter mBluetoothAdapter;
    private ListView mBTDeviceListView;
    private BT_SEARCH_STATE mBTSearchingState;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBTDeviceListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                ...
                intent.putExtra("DEVICE_ADDR", device.getAddress());
                Log.d(TAG, "you selected : " + device.getAddress());
                setResult(RESULT_OK, intent);
                startActivityForResult(intent,RESULT_OK);
            }
        });

        mBTSearchingState = BT_SEARCH_STATE.IDLE;

        mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
        if(null == mBluetoothAdapter)
            Toast.makeText(getApplicationContext(),"doesn't support Bluetooth",Toast.LENGTH_SHORT).show();
    ...
    private void updateDeviceList(){
        ...
        if (hasPermission != PackageManager.PERMISSION_GRANTED) {
            ...
            Toast.makeText(getApplicationContext(), "start discovering", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch(item.getItemId())
        {
            ...
            {
                if(mBTSearchingState == BT_SEARCH_STATE.IDLE) {
                    if (mBluetoothAdapter.isDiscovering()) {
                        mBluetoothAdapter.cancelDiscovery();
                    }
                    updateDeviceList();
                }
                else if(mBTSearchingState == BT_SEARCH_STATE.SEARCHING) {
                    if (mBluetoothAdapter.isDiscovering()) {
                        mBluetoothAdapter.cancelDiscovery();
                    }
                }
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
               ...
               Log.d(TAG, "BT device found:" + device.getName() + device.getAddress());
               ...
               Log.d(TAG, "BT device discover started");
                mBTSearchingState = BT_SEARCH_STATE.SEARCHING;
                updateUI();
